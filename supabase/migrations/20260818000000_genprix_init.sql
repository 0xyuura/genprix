...
--
-- So: strip everything from all three, then grant back the seven the client
-- actually calls.
do $$
declare f record;
begin
...
-- carries a run token, or checks the admin passcode itself.
...
--   room_ttl / room_capacity / room_is_live — internals. answer_matches in
--                        particular would let anyone test candidate answers offline.
...
--
-- Then open /admin in the app, paste the ten questions, hit Save, and create a
-- room. This file is safe to re-run: tables use IF NOT EXISTS, functions use
-- CREATE OR REPLACE, and every policy is dropped before it is recreated.
-- ============================================================================
